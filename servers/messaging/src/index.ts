...
const main = async () => {
  ...
  app.use("/v1/channels/:channelID/members", async (req: Request, res: Response) => {
  ...
  app.use("/v1/channels/:channelID", async (req: Request, res: Response) => {
    const { headers, params, method, body } = req;
    // We can safely assume that x-user is populated sinced we ran an authentication check
    const user = getXUser(headers);
  ...
  app.use("/v1/channels", async (req: Request, res: Response) => {
    const { headers, params, method, body } = req;
    // We can safely assume that x-user is populated sinced we ran an authentication check
    const user = getXUser(headers);
  ...
  app.use("/v1/messages/:messageID", async (req: Request, res: Response) => {
    const { headers, params, method, body } = req;
    // We can safely assume that x-user is populated sinced we ran an authentication check
    const user = getXUser(headers);
  ...
  const createChannel = (req: Request, creator: User): Channel => {
    const { body } = req;
    const c = body;

    c.members.push(creator.ID);
    return new Channel("", c.name, c.description, c.private, c.members, c.createdAt, creator, c.editedAt);
  };

  const createMessage = (req: Request, creator: User): Message => {
    const { body, params } = req;
    const m = body;
    return new Message("", params.channelID, m.createdAt, m.body, creator, m.editedAt);
  };
