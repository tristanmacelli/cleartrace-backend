...
const recursiveCreateConnection = async (retryInterval: number): Promise<void | MongoClient> => {
  ...
  try {
    ...
    console.log(`mongo_handlers createConnection ${e}`);
...
export const getChannels = async (channels: Collection, userID: number, search: string): Promise<{
...
export const insertNewChannel = async (channels: Collection, newChannel: Channel): Promise<{
  newChannel: Channel;
  err: Error | null;
}> => {
    // Duplicate Check
...
export const insertNewMessage = async (messages: Collection, newMessage: Message): Promise<{
  newMessage: Message;
  err: Error | null;
}> => {
    const insertDoc = {
    ...
    const insertResult = await messages.insertOne(insertDoc)
      .then((res) => res)
      .catch((reason) => {
        console.log(`mongo_handlers insertNewMessage ${reason}`)
        return new Error(reason)
      });
...
const updateChannelMembers = async (channels: Collection, existingChannel: Channel, newMembersList: number[]) => {
  ...
  const updateError = await channels.updateOne(filter, updateDoc)
    .then(() => null)
    .catch((reason) => {
      console.log(`mongo_handlers updateChannelMembers ${reason}`)
      return new Error(reason)
    });
...
export const updateChannel = async (
  ...
  const updateError = await channels.updateOne(filter, updateDoc)
    .then(() => null)
    .catch((reason) => {
      console.log(`mongo_handlers updateChannel ${reason}`)
      return new Error(reason)
    });
...
export const updateMessage = async (messages: Collection, existingMessage: Message, updates: { body: string }) => {
    ...
    const updateError = await messages.updateOne(filter, updateDoc)
      .then(() => null)
      .catch((reason) => {
        console.log(`mongo_handlers updateMessage ${reason}`)
        return new Error(reason)
      });
...
export const deleteChannel = async (channels: Collection, messages: Collection, existingChannel: Channel): Promise<Error | null> => {
    ...
    const channelDeletionError = await channels.deleteOne(channelFilter)
      .then(() => null)
      .catch((reason) => {
        console.log(`mongo_handlers deleteChannel ${reason}`)
        return new Error(reason)
      });
    if (channelDeletionError) return channelDeletionError;

    const messageFilter = { channelID: existingChannel.id }
    const messagesDeletionError = await messages.deleteMany(messageFilter)
      .then(() => null)
      .catch((reason) => {
        console.log(`mongo_handlers deleteChannel ${reason}`)
        return new Error(reason)
...
export const deleteMessage = async (messages: Collection, existingMessage: Message): Promise<Error | null> => {
  ...
  const deleteError = await messages.deleteOne(filter)
    .then(() => null)
    .catch((reason) => {
      console.log(`mongo_handlers deleteMessage ${reason}`)
      return new Error(reason)
    });
...
export const getChannelByID = async (channels: Collection, id: string): Promise<{ channel?: Channel, err: Error | null }> => {
    ...
    const result = await channels.findOne(filter)
      .catch((reason) => {
        console.log(`mongo_handlers getChannelByID ${reason}`)
        return new Error(reason)
      });
...
export const getMessageByID = async (messages: Collection, id: string): Promise<{ message?: Message, err: Error | null }>  => {
    const filter = { _id: new ObjectId(id) };
    // Since id's are auto-generated and unique we chose to use findOne() instead of find()
    const result = await messages.findOne(filter)
      .catch((reason) => {
        console.log(`mongo_handlers getMessageByID ${reason}`)
        return new Error(reason)
      });
...
export const last100Messages = async (messages: Collection, channelID: string, messageID: string): Promise<{
